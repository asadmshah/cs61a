...
def add_interval(x, y):
	...
	'3 to 10'
	"""
	int1 = lower_bound(x) + lower_bound(y)
	int2 = upper_bound(x) + upper_bound(y)
...
def div_interval(x, y):
	...
	'-0.25 to 0.5'
	"""
	assert upper_bound(y) - lower_bound(y) > 0, "Interval spans 0"
	reciprocal_y = interval(1/upper_bound(y), 1/lower_bound(y))
	return mul_interval(x, reciprocal_y)

def sub_interval(x, y):
	"""
	Return the interval that contains the difference between any value in x and
	any value in y.

	>>> str_interval(sub_interval(interval(-1, 2), interval(4, 8)))
	'-9 to -2'
	"""
	n1 = lower_bound(x) - lower_bound(y)
	n2 = lower_bound(x) - upper_bound(y)
	n3 = upper_bound(x) - lower_bound(y)
	n4 = upper_bound(x) - upper_bound(y)
	return interval(min(n1,n2,n3,n4), max(n1,n2,n3,n4))
...
def width(x):
	...
	"""
	return (upper_bound(x) - lwoer_bound(x)) / 2
...
def sum_nonzero_with_map_filter_reduce(seq):
	"""
	Returns an interval that is the sum of the squares of the non-zero
	intervals in sequence, using map, filter, and reduce.
...
def sum_nonzero_with_generator_reduce(seq):
	"""
	Returns an interval that is the sum of the squares of the non-zero
	intervals in seq, using reduce and a generator expression.
